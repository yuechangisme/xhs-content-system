...
- 后续新帖暂时默认做成 5 页版本：封面 1 页 + 内容 3 页 + 总结 1 页，除非用户明确要求 7 页。
...
1. 页数是否为 5 页：P1 封面、P2-P4 内容、P5 总结。
...
6. 是否保持暖白底、轻分隔线、轻编号、淡绿色高亮；不得回到 V6.1 卡片网格、深绿内页或方案卡结构。
...
3. 默认 8 篇循环为：懒人饮食手册 → 身体微习惯手册 → 打工人恢复手册 → 懒人运动手册 → 懒人饮食手册 → 身体微习惯手册 → 打工人恢复手册 → 好物避坑手册。
4. 先检查最近的 `待制作` / `待投递` / `已投递` 内容，尽量判断上一篇所属合集；无法可靠判断时，说明不确定，并默认从「懒人饮食手册」开始新一轮。
5. 自动提示本次轮到的合集、适合方向和 3 个候选选题，再等用户确认方向。
6. 如果用户直接指定明确选题，则优先尊重用户选题，只说明它归属哪个合集。
7. 不得手动修改 `state.json` 来记录或伪造轮转状态。
