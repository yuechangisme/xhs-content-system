...
- **v0.5 MVP 支持 Tier 0**：本地/人工输入 + 节气/季节节点 + 手动热点导入 + 手动数据录入
...
| v0.5.4 | 发布后数据回流 | 手动录入互动指标（点赞/收藏/评论/阅读）、评论信号提取、数据反推选题方向、不做自动抓取 |
...
### v0.5.4 Post Performance Analytics Contract

#### 目标

从已发布内容的互动数据（点赞/收藏/评论/阅读）中反推选题方向，为下一轮策划提供数据支撑。

**不改变现有管线，不修改 state.json，不替代 TopicCandidate 流程。**

#### 数据回流来源分级

| 级别 | 方式 | 可获取的数据 | 风险 | v0.5.4 MVP |
|------|------|-------------|------|-----------|
| **Tier 0** | 手动查看 App/创作者后台后录入 | 点赞数、收藏数、评论数、阅读量、评论内容 | 无 | **P0** 主推 |
| **Tier 1** | 手动导出 CSV/Excel 后导入（未来） | 详细趋势数据、粉丝画像 | 低 | P1 扩展目标 |
| **Tier 2** | 登录态只读采集（Puppeteer 读后台） | 同上，但自动获取 | 中 | **暂缓** |
| **Tier 3** | 小红书爬虫 / API 自动抓取 | 全量评论、互动趋势、竞品对比 | 高 | **暂缓** |

**v0.5.4 MVP 只做 Tier 0：** 用户打开小红书 App 或创作者后台，人工查看数据后手动录入。

#### PostMetrics 数据结构

```json
{
  "postId": "投稿内容/已投递/2026-05-17-内脏脂肪食物",
  "taskDir": "2026-05-17-内脏脂肪食物",
  "title": "内脏脂肪最怕8种家常食物",
  "publishedAt": "2026-05-17T12:42:28.237Z",
  "topicSource": "seasonal",
  "topicTitle": "立夏后最该吃的3种食材",
  ...
  "commentSignals": [
    {
      "keyword": "内脏脂肪怎么减",
      "evidence": "多条评论问具体方法",
      "suggestedTopic": "内脏脂肪减脂的懒人方法"
    }
  ],
  "notes": "收藏率高，说明内脏脂肪话题有强需求"
}
```

| 字段 | 必填 | 说明 |
|------|------|------|
| `postId` | ✅ | 对应 state.json 中的 post id |
| `taskDir` | ✅ | 目录名，关联 content/投稿内容/ |
| `title` | ✅ | 帖子标题 |
| `publishedAt` | ✅ | 发布时间 |
| `topicSource` | 可选 | 选题来源（seasonal/manual/old） |
| `topicTitle` | 可选 | 对应 TopicCandidate 标题 |
| `recordedAt` | ✅ | 数据记录时间 |
| `metrics` | ✅ | 指标对象 |
| `engagement` | 可选 | 衍生指标（系统自动计算） |
| `commentSignals` | 可选 | 评论中提取的选题信号 |
| `notes` | 可选 | 人工复盘备注 |

**衍生指标计算规则：**
- `likeRate = likes / views`（views 为 0 时取 0）
- `favRate = favorites / views`
- `commentRate = comments / views`

#### analytics 与现有文件的边界

```text
state.json                ← 管线状态（QA/发布/排期）
analytics/post-metrics.json  ← 发布后数据（互动指标、评论）
topics/candidates.json    ← 选题候选池
content/投稿内容/           ← 物理内容归档
```

**职责分离：**
- `state.json` — 只记录帖子运行状态，不存指标数据
- `analytics/post-metrics.json` — 只存发布后数据，不存管线状态
- `topics/candidates.json` — 只存选题候选，不存已发布数据
- `content/投稿内容/` — 只存物理文件，不存分析状态

**约束：**
- analytics 不得写 state.json
- analytics 不得直接修改 topic pool
- analytics 不得修改 content/投稿内容/

#### analytics → TopicCandidate 转换规则

```
用户查看 analytics summary
    ↓ 人工发现有价值的选题方向
用户通过 topic add 手动录入 TopicCandidate
    ↓
CANDIDATE → SHORTLISTED → APPROVED → EXPORTED → xhs-planner
```

**关键约束：**
- analytics 不直接生成 TopicCandidate
- analytics 最多输出 commentSignals 中的 `suggestedTopic` 建议
- `suggestedTopic` 是否采纳由用户判断
- 无论数据多好，仍走完整 CANDIDATE 流程
- analytics 不能绕过 topic add / shortlist / approve

#### MVP CLI 设计（不实现，仅合约）

```bash
# 手动录入指标数据
node pipeline.js analytics add \
  --taskDir "2026-05-17-内脏脂肪食物" \
  --views 12500 \
  --likes 892 \
  --favorites 2341 \
...
# 查看所有记录
node pipeline.js analytics list

# 数据分析摘要
node pipeline.js analytics summary
```

#### 禁止项（v0.5.4）

- 不自动登录小红书创作者后台
- 不读取 cookie、不读取 session
- 不使用 Puppeteer 读取后台数据
- 不自动抓取评论
- 不自动生成 TopicCandidate
- 不自动发布
- 不修改 state.json
- 不修改 topics/candidates.json
- 不修改 content/投稿内容/
